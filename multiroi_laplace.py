...
parser.add_argument("--distmax",help="distance max",type=int,default=100)
...
I_origineLaplacian=cv2.Laplacian(I_origine,cv2.CV_64F)
...
def AnalyzeImage(f_image):
	...
	I_destLaplacian = cv2.Laplacian(I_dest,cv2.CV_64F)
	I_show = cv2.cvtColor(I_dest,cv2.COLOR_GRAY2BGR)
	for (x0,y0,x1,y1,DX,DY,d) in Grillage(I_origineLaplacian,I_destLaplacian,l_rois):
		if d<threshold and (DX*DX+DY*DY) < distmax:
			global hit
			hit = hit+1
...
print "hit : " + str(hit) + "  - files : " + str(len(files)) + "  " + str(hit/len(files))
